...
module('Microsoft Live - Integration', {
  setup: function(){
    ...
    configure({
      providers: {
        'microsoft-live-oauth2': { apiKey: 'dummy' }
      }
    });
  },
  teardown: function(){
...
test("Opens a popup to Microsoft Live", function(assert){
  Ember.run(function(){
    torii.open('microsoft-live-oauth2').finally(function(){
...
test('Validates the state parameter in the response', function(assert){
  ...
  Ember.run(function(){
    torii.open('microsoft-live-oauth2').then(null, function(e){
